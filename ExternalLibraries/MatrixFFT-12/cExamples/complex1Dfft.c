 ...
 * 
 * complex1D.c.cpp - simple 1-D complex FFT example.
 *
 * Created Oct. 29 2009. 
 */
...
#define SAMPLES_DEF			32

static void usage(char **argv)
{
	printf("usage: %s [options]\n", argv[0]);
	printf("Options:\n");
	printf("  -s samples      -- # complex samples; default %u\n", SAMPLES_DEF);
	exit(1);
}

int main(int argc, char **argv)
{
	size_t          numSamples = SAMPLES_DEF;
     ...
     */
    alignedBuf = fftAllocComplexArrayAlign(numSamples, FFT_MEM_ALIGNMENT, &freeBuf);
    if(alignedBuf == NULL) {
    ...
    fftDump1DComplex("Original data", alignedBuf, numSamples);
    ...
    fftDump1DComplex("Forward FFT output", alignedBuf, numSamples);
    ...
    fftDump1DComplex("Inverse FFT output", alignedBuf, numSamples);
