 ...
 * 
 * complex1D.c.cpp - simple 2-D real-signal FFT example.
 *
 * Created Oct. 29 2009. 
 */
...
int main(int argc, char **argv)
{
	...
	size_t          totalRealSamples = 0;
	size_t          totalComplexSamples = 0;
     ...
     */
    totalRealSamples = numRows * numCols;
    totalComplexSamples = totalRealSamples >> 1;
    alignedBuf = fftAllocComplexArrayAlign(totalComplexSamples, FFT_MEM_ALIGNMENT, &freeBuf);
    if(alignedBuf == NULL) {
     ...
     */
        ...
        for(dex=0; dex<totalRealSamples; ) {
            size_t bufDex = dex/2;
    ...
    fftDumpMatrixReal("Original data", alignedBuf, numRows, numCols);
    ...
    fftDumpMatrixReal("Forward FFT output", alignedBuf, numRows, numCols);
    ...
    fftDumpMatrixReal("Inverse FFT output", alignedBuf, numRows, numCols);
