function Y = ode4k(odefun,kappa,ymin,ymax,tspan,y0,varargin)
...
kappa = kappa(:); % Make it a column vector
if length(kappa) == 1 && length(y0) > 1 % if it's a scalar, make it a vector
    kappa = kappa*ones(size(y0));
end
if ~isequal(size(kappa),size(y0))
  error('Inconsistent sizes of Y0 and kappa.');
end

ymin = ymin(:);
ymax = ymax(:);
if ~isequal(size(y0),size(ymin)) || ~isequal(size(y0),size(ymax))
...
for i = 1:length(ymin)
   if kappa(i) == 0
    ...
    elseif ymin(i) == -Inf && isfinite(ymax(i))
       ...
       b = ymax(i);
       s = sqrt(2*kappa(i));
       f{i} = @(x,dt) -abs((s/sqrt(dt))*randn(1,1) - (b-x)/dt) + (b-x)/dt;
   elseif isfinite(ymin(i)) && ymax(i) == Inf
       % bounded below
       a = ymin(i);
       s = sqrt(2*kappa(i));
       f{i} = @(x,dt) abs((s/sqrt(dt))*randn(1,1) - (a-x)/dt) + (a-x)/dt;
   elseif isfinite(ymin(i)) && isfinite(ymax(i))
       % bounded above and below
       a = ymin(i);
       b = ymax(i);
       s = sqrt(2*kappa(i));
       f{i} = @(x,dt) abs( mod( (s/sqrt(dt))*randn(1,1)-(b-x)/dt, 2*(b-a)/dt) - (b-a)/dt ) + (a-x)/dt;
   elseif ymin(i) == -Inf && ymax(i) == Inf
       % unbounded
       f{i} = @(x,dt) sqrt(2*kappa(i)/dt)*randn(1,1);
   else
