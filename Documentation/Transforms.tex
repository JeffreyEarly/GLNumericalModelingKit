...
\begin{equation}
a^i \delta^i_{j+1} = a^{i-1} \delta^{i-1}_j = \left[\begin{array}{cccc}0 & 0 & 0 & 0 \\a_1 & 0 & 0 & 0 \\0 & \ddots & 0 & 0 \\0 & 0 & a_{n-1} & 0\end{array}\right]
\end{equation}
...
We denote the $k$ frequency component in the sine basis by $h_k^{\textrm{s}}$.
\begin{align}
\partial_x H_n &= \partial_x  \left( (-1)^n h_{N-1} + 2 \sum_{k=0}^{N-2} h_k^{\textrm{s}} \sin \left[ 2 \pi f_k^{\textrm{s}} x_n \right] \right)\\
&= 2 \sum_{k=0}^{N-2} h_k^{\textrm{s}} \left( 2 \pi f_k^{\textrm{s}} \right) \cos \left[ 2 \pi f_k^{\textrm{s}} x_n \right]
\end{align}
Now let $k=m-1$, so that
\begin{align}
\partial_x H_n &= 2 \sum_{m=1}^{N-1}  h_k^{\textrm{s}} \left( 2 \pi f_k^{\textrm{s}} \right)\cos \left[ 2 \pi f_{m-1}^{\textrm{s}} x_n \right] \\
&= 2 \sum_{m=1}^{N-1}  h_k^{\textrm{s}} \left( 2 \pi f_k^{\textrm{s}} \right) \cos \left[ 2 \pi f_m^{\textrm{c}} x_n \right]
\end{align}
where we used that $f_k^{\textrm{s}}=f_{k+1}^{\textrm{c}}$ in the last step. Let's define $g_m^{\textrm{c}}$ to be the coefficient of the sum. In that case,
\begin{equation}
g_m^{\textrm{c}} = \delta_{m-1}^k h_k^{\textrm{s}} \left( 2 \pi f_k^{\textrm{s}} \right).
\end{equation}
This means that the differentiation operator $D_x^{\textrm{s}}$ is defined as,
\begin{equation}
D_x^{\textrm{s}} \equiv \left( 2 \pi f_k^{\textrm{s}} \right) \delta_{m-1}^k.
\end{equation}
