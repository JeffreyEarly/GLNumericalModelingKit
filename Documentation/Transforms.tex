...
NOTE: NEED TO ADD THE NORMALIZATIONS. We normalize the forward transform (rather than the traditional inverse transform), so that it is not length dependent.
...
In the current implementation (as of March 5th, 2015) both the fast transform and discrete transform return the same normalization, except for the $T_0$. The fast transform is a factor of two larger.
